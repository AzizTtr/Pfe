...
--  Voir T:\pfe\pfe\database\schema.sql pour la version annotée
-- =========================================================================

-- ─────────────────────────────────────────────────────────────────────────
-- 1. RÔLES & UTILISATEURS (auth déléguée à Keycloak)
-- ─────────────────────────────────────────────────────────────────────────
...
CREATE TABLE evaluation_requests (
    ...
    status                ENUM(
                            'DRAFT','PENDING_REVIEW','UNDER_EVALUATION','INFO_REQUESTED',
                            'REJECTED_INITIAL','APPROVED_INITIAL','PENDING_ADMIN','REJECTED_ADMIN',
                            'APPROVED_ADMIN','PENDING_FIELD','REJECTED_FINAL','APPROVED_FINAL','COMPLETED'
...
CREATE TABLE evaluation_answers (
    ...
    initial_value_id            BIGINT NULL,
    final_value_id              BIGINT NULL,
    answer_text                 TEXT,
    evaluator_note              TEXT,
...
-- Vues pour le dashboard analytique

CREATE OR REPLACE VIEW v_request_summary AS
SELECT
    er.id, er.request_number, er.status, er.final_percentage,
    gs.label_ar AS grade_ar, gs.label_en AS grade_en,
    ee.name AS entity_name, ee.country, ee.city,
    er.submitted_at, er.updated_at
